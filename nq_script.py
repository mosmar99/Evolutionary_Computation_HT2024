...
def init_population_generation(n, population_size):
    # low while is included, however, high value is excluded in the calcs
    return np.random.randint(low=1, high=n+1, size=(population_size, n))
...
def eval_curr_fitness(state, population_size):
    fitness_evals = []
    n = len(state[0])
    max_pairs = n * (n - 1) / 2  # Maximum number of pairs of queens
    for i in range(population_size):
        fitness_evals.append(1 - fitness_function(state[i]) / max_pairs) 
...
def mutation(state, mutation_prob):
    ...
    for board in state:
        if(np.random.rand() < mutation_prob):
            new_gene = np.random.randint(1, n+1)
...
def visualize_board(state):
    n = len(state)
    columns = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"[:n]
    print("\n")
...
def solve_n_queens_evolutionary (n=8, population_size=100, crossover_rate=0.8, mutation_prob=0.3):

    init_state = init_population_generation(n, population_size)
